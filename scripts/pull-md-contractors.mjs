...
function parseMHIC(html) {
  ...
  while ((rowMatch = rowRegex.exec(html)) !== null) {
    ...
    if (cells.length >= 5 && rowCount > 0 && !/license/i.test(cells[0])) {
      contractors.push({ license: cells[0], name: cells[1], trade_name: cells[2], city: cells[3], county: cells[4], status: cells[5] || "" });
    }
    rowCount++;
  }
  return contractors;
...
for (let i = 0; i < MD_ZIPS.length; i += BATCH) {
  ...
  process.stdout.write(`Querying zips ${i + 1}–${Math.min(i + BATCH, MD_ZIPS.length)} of ${MD_ZIPS.length}...`);
...
const rows = all.map(r => [r.license, r.name, r.trade_name, r.city, r.county, r.status].map(escape).join(","));
const csv = [header, ...rows].join("\r\n");

const date = new Date().toISOString().slice(0, 10);
const filename = `maryland-contractors-${date}.csv`;
writeFileSync(filename, csv);
console.log(`\nDone! ${all.length} contractors saved to ${filename}`);
